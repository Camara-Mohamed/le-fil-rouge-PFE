...
class PresentationSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if ($lea)     TrainingRegister::create(['training_id' => $tA->id, 'user_id' => $lea->id,     'status' => RegisterStatus::ACCEPTED]);
        if ($luc)     TrainingRegister::create(['training_id' => $tA->id, 'user_id' => $luc->id,     'status' => RegisterStatus::PENDING, 'notes' => 'Doit confirmer avant jeudi.']);
        ...
        if ($lea)     TrainingRegister::create(['training_id' => $tB->id, 'user_id' => $lea->id,     'status' => RegisterStatus::ACCEPTED]);

        TrainingRegister::create(['training_id' => $tC->id, 'user_id' => $jBrevete->id,'status' => RegisterStatus::ACCEPTED]);
        if ($tiffany) TrainingRegister::create(['training_id' => $tC->id, 'user_id' => $tiffany->id, 'status' => RegisterStatus::ACCEPTED]);
        if ($luc)     TrainingRegister::create(['training_id' => $tC->id, 'user_id' => $luc->id,     'status' => RegisterStatus::REFUSED, 'notes' => 'Absent ce soir-là.']);
        ...
        if ($lea)     CampRegister::create(['camp_id' => $cA->id, 'user_id' => $lea->id,     'status' => RegisterStatus::ACCEPTED]);
        if ($tiffany) CampRegister::create(['camp_id' => $cA->id, 'user_id' => $tiffany->id, 'status' => RegisterStatus::ACCEPTED]);

        CampRegister::create(['camp_id' => $cC->id, 'user_id' => $jAnim1->id,  'status' => RegisterStatus::PENDING]);
        if ($luc)     CampRegister::create(['camp_id' => $cC->id, 'user_id' => $luc->id,     'status' => RegisterStatus::ACCEPTED]);
        ...
        foreach ($trainings->take(5) as $i => $training) {
            ...
            if ($a1) Comment::create(['content' => $formComments[$i % count($formComments)],       'user_id' => $a1->id, 'training_id' => $training->id, 'is_admin' => false]);
            if ($a2) Comment::create(['content' => $formComments[($i+1) % count($formComments)],   'user_id' => $a2->id, 'training_id' => $training->id, 'is_admin' => false]);
            if ($a3) Comment::create(['content' => $formComments[($i+2) % count($formComments)],   'user_id' => $a3->id, 'training_id' => $training->id, 'is_admin' => false]);
        }
        ...
        foreach ($camps->take(5) as $i => $camp) {
            $a1 = $members->get($i % $members->count());
            $a2 = $members->get(($i + 1) % $members->count());
            if ($a1) Comment::create(['content' => $campComments[$i % count($campComments)],     'user_id' => $a1->id, 'camp_id' => $camp->id, 'is_admin' => false]);
            if ($a2) Comment::create(['content' => $campComments[($i+1) % count($campComments)], 'user_id' => $a2->id, 'camp_id' => $camp->id, 'is_admin' => false]);
        }
