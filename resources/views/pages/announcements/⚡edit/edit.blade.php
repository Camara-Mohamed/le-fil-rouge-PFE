    ...
    <div class="flex flex-col gap-6 px-4 py-8 md:px-8">
        ...
        <form wire:submit="save" class="flex flex-col gap-6">
            ...
            <section class="p-6 bg-white rounded-2xl shadow-[0px_5px_20px_0px_rgba(0,0,0,0.10)] flex flex-col gap-6">
                            ...
                            @foreach($announcement->galeries as $galerie)
                                <div wire:key="galerie-{{ $galerie->id }}" class="relative group">
                                    ...
                                    <button type="button"
                                        ...
                                        {{ __('pages/announcements.gallery_delete') }}
                                    </button>
                                </div>
                            @endforeach
                        </div>
                ...
                {{-- Nouvelle galerie --}}
                <div class="flex flex-col gap-3">
                    <span class="font-sans font-bold text-base text-dark">{{ __('pages/announcements.label_new_gallery') }}</span>
                    @if($form->galeries)
                        <div class="grid grid-cols-3 md:grid-cols-4 gap-3">
            ...
            <button type="submit"
                ...
                {{ __('pages/announcements.edit_btn_submit') }}
            </button>
        ...
        @can('delete', $announcement)
            <button type="button" wire:click="openConfirmDeleteModal"
                ...
                {{ __('pages/announcements.delete_btn') }}
            </button>
        @endcan
