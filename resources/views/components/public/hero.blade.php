...
@if($banner)
    <div class="relative h-64">
        ...
        <div class="absolute inset-0 flex flex-col justify-end px-4 md:px-8 pb-8 gap-2">
            @if($title)
                <h2 class="font-sans font-black text-3xl md:text-5xl text-white [text-shadow:0_2px_8px_rgba(0,0,0,0.4)]">
                    {{ $title }}
                </h2>
            @endif
            @if($subtitle)
...
@else
    <div class="px-4 md:px-8 py-24 bg-bg border-b border-bg-dark">
        @if($title)
            <h2 class="font-sans font-black text-4xl md:text-5xl text-dark">{{ $title }}</h2>
        @endif
        @if($subtitle)
